        ...
        link: function linkFn($scope, $elem, $attrs) {
          ...
          function initSticky() {
            ...
            if (shouldInitialize) {
              ...
              // Initialize as unsticky. If an element is sticky, it will be handled below.
              unStickElement();
          ...
          var checkIfShouldStick = function() {
            ...
            var scrollFromDirection = getScrollFromDirection(scrollbarPosition);
            var shouldStick;
            ...
            if (shouldStick && !shouldStickWithLimit($attrs.stickLimit) && !isSticking) {
              stickElement(scrollFromDirection);
            } else if (!shouldStick && isSticking) {
              unStickElement(scrollFromDirection);
            } else if (confine && !shouldStick && isSticking) {
              unStickElement(scrollFromDirection);
            }
          };

          /**
           * Get the scroll direction. Down the page is 'top', up the page is 'bottom'
           */
          function getScrollFromDirection(scrollbarPosition) {
              var fromDirection = scrollbarPosition < lastScrollTop ? 'top' : 'bottom';
