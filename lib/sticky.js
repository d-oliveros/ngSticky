      ...
      return {
        ...
        link: function linkFn($scope, $elem, $attrs) {
          ...
          $scope.onResize = function () {
            ...
            if (isSticking) {
              $elem.css('width', $scope.computeOffsetWidth(elem) + 'px');
            }
          };
          ...
          $scope.computeParentOffsetWidth = function (currentElem) {
            var parent = $window.getComputedStyle(currentElem.parentElement, null);
            return currentElem.parentElement.offsetWidth -
              parent.getPropertyValue('padding-right').replace('px', '') -
              parent.getPropertyValue('padding-left').replace('px', '');
          };

          $scope.computeOffsetWidth = function (currentElem) {
            if (currentElem === undefined) {
              return 0;
            }
            if (currentElem.offsetWidth === 0) {
              // Get parent offset width
              var parentOffsetWidth = $scope.computeParentOffsetWidth(currentElem);

              if (parentOffsetWidth === 0) {
                var parent = $window.getComputedStyle(currentElem.parentElement, null);
                return $scope.computeOffsetWidth(parent);
              } else {
                return parentOffsetWidth;
              }
            } else {
              return currentElem.offsetWidth;
            }
          };
          ...
          $scope.stickElement = function () {
            ...
            var offsetWidth = $scope.computeOffsetWidth(elem);

            initialCSS.offsetWidth = offsetWidth;
